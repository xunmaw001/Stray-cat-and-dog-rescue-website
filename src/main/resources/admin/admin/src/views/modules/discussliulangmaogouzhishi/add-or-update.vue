...
export default {
    ...
    info(id) {
      this.$http({
        url: `discussliulangmaogouzhishi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discussliulangmaogouzhishi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discussliulangmaogouzhishiCrossAddOrUpdateFlag = false;
						 this.parent.search();
